...
clientFactory.createConfigFromUrl('https://arn-server-test-dev.arianee.com/testproject').then(arnConfig => {
  arnConfig.auth.signCallback = async () => window.confirm('Ok to sign ?');
  const arnClient = clientFactory.create(arnConfig);
  console.log('Created ARN client with auth config', arnClient.auth.config);
